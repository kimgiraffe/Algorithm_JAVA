 ...
 * n = 2인 경우... 2*1 타일 2개 또는 1*2 타일 2개, 2*2 타일 1개로 총 3가지 방법으로 타일을 채울 수 있다.
 * n >= 3인 경우... 
 ...
 * 맨 왼쪽 2*2 타일을 2*2 타일 1개를 사용하여 채웠다면 나머지 타일을 채우는 방법은 n-2일 때 방법의 수와 같다.
 * f(n) = f(n-1) + 2 * f(n-2) 
 * @author semin.kim
 *
 */
...
public class BOJ_11727_2xn타일링2_김세민 {
	...
	private static void fill() {
		...
		fillTiles[2] = 3; // n = 2인 경우... 2*1 타일 2개 또는 1*2 타일 2개, 2*2 타일 1개로 총 3가지 방법으로 타일을 채울 수 있다.
		...
		for(int idx = 3; idx <= MAX_TILE_COUNT; idx++) {
			if(fillTiles[idx] == 0) {
				fillTiles[idx] = (fillTiles[idx-1] % MOD + 2 * fillTiles[idx-2] % MOD) % MOD; 
			}
		}
